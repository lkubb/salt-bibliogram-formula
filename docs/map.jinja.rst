...
We will see a quick setup to configure the ``bibliogram`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``bibliogram`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``bibliogram`` formulas under ``/srv/salt/bibliogram/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/bibliogram/parameters/dns:domain/
    ln -s dns:domain /srv/salt/bibliogram/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/bibliogram/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/bibliogram/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``bibliogram`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``bibliogram`` formulas under ``/srv/salt/bibliogram/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/bibliogram/parameters/roles
...
We create a configuration for the role ``bibliogram/server`` in ``/srv/salt/bibliogram/parameters/roles/bibliogram/server.yaml``:
...
We create another configuration for the role ``bibliogram/client`` in ``/srv/salt/bibliogram/parameters/roles/bibliogram/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles bibliogram/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles bibliogram/client
...
.. code-block:: console
    ...
    mv /srv/salt/bibliogram/parameters/dns:domain/  '/srv/salt/bibliogram/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-bibliogram-as-json:
      test.nop:
        - name: {{ bibliogram | json }}
...
Here is an example based on `template-formula/bibliogram/config/file.sls`_:
    ...
    bibliogram-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='bibliogram-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ bibliogram.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``bibliogram`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ bibliogram['winner'] }}
...
.. _template-formula/bibliogram/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/bibliogram/config/file.sls
